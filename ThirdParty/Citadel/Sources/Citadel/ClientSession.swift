...
final class SSHClientSession: Sendable {
    ...
    /// - settings: The settings to use for the SSH session.
    #if canImport(Network)
    /// Shared Network.framework event-loop group for client connections on Apple
    /// platforms, so sockets follow the system VPN/cellular routing (e.g. Tailscale).
    static let transportServicesGroup = NIOTSEventLoopGroup()
    #endif
    ...
    static func connect(
        ...
        #if canImport(Network)
        // Apple: connect via Network.framework (NIO Transport Services) so the socket
        // follows the system VPN/cellular routing. Plain BSD sockets (ClientBootstrap)
        // don't route reliably over a VPN on iOS — this is why Termius connects over
        // Tailscale-on-cellular and the old socket path timed out.
        connectFuture = NIOTSConnectionBootstrap(group: Self.transportServicesGroup)
        ...
        connectFuture = ClientBootstrap(group: eventLoop).channelInitializer { channel in
            return Self.addHandlers(on: channel, inboundChannelHandler: inboundChannelHandler, settings: settings)
        }
        .connectTimeout(settings.connectTimeout)
        ...
        return try await connectFuture.flatMap { channel in
